...
int main(int argc, char **argv){
  std::string ppn_key = "+ppn";
  int num_thread = -1;
  for (int i = 0; i < argc; ++i) {
    std::string s = argv[i]; // type cast char to string
    // If ppn_key is in argc, get thread size
    size_t pos = std::string::npos;
    if ((pos = s.find(ppn_key)) != std::string::npos) {
      std::string num_thread_s = s.erase(pos, ppn_key.length());
      num_thread = stoi(num_thread_s);
    }
  }
  #pragma omp parallel 
  {
    ...
    if (peid == 0) {
        printf("[DIAGONALIZER] Setting OMP_NUM_THREADS=%i\n", num_thread);
    ...
    for ( ; iternum < total_iter; iternum++) {
      ...
      nb      =  diagData->nb;
      jobz    = 'V';
      uplo    = 'U'; // upper diagonal symmetric matrix
      ...
      // Resize rwork array 
      lrwork = static_cast<int>(rwork[0]) * 2; // Bug in Scalapack, use 2 here!
      free(rwork);
